...
def get_system_prompt(specialist_id: str) -> str:
    """Return the system prompt for a specialist."""
...
def filter_context_for_specialist(
    ...
    """
    Filter context sources to only those the specialist is authorised to see.
    If persona_ids_per_source is provided (doc_id -> [persona_ids]), only include
    sources where this specialist is in the list. Otherwise use type-based permissions.
    """
    ...
    for src in sources:
        ...
        # If we have per-document persona access, check it
